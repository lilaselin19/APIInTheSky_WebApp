...
class incident{
  constructor(cad_incident_ID,incident_datetime,final_call_type, dispatch_response_seconds_qy, borough, zipcode){
    this.cad_incident_ID = cad_incident_ID;
    ...
    for(var i = 0; i <rows.length; i++){
      ...
      console.log(zipcode.trim());
        if(rows[i].zipcode == zipcode){
          incident = new incident(rows[i].cad_incident_ID, rows[i].incident_datetime, rows[i].final_call_type, rows[i].dispatch_response_seconds_qy, rows[i].borough, rows[i].zipcode);
...
exports.getbyID = function(cad_incident_ID, callback){
  var incident = {};
  var incidentlist = [];
    exports.rows(function(rows){
    for(var i = 0; i <rows.length; i++){
      console.log(rows[i].cad_incident_ID.trim());
      console.log(cad_incident_ID.trim());
        if(rows[i].cad_incident_ID == cad_incident_ID){
          incident = new incident(rows[i].cad_incident_ID, rows[i].incident_datetime, rows[i].final_call_type, rows[i].dispatch_response_seconds_qy, rows[i].borough, rows[i].zipcode);
          incidentlist.push(Object.entries(incident));
        }
    }
